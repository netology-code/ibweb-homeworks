...
2\. В адресной строке Google Chrome вбейте адрес `chrome://certificate-manager/`, и вы попадёте на страницу, как на скрине.

![](pic/01.png)


3\. Откройте меню "Настроить сертификаты, импортированные из (название ОС)". Откроется системное меню по управлению сертификатами.
Для ОС Windows будет открыто окно:

![](pic/02.png)

Нужно нажать кнопку "Импорт" и запустить мастер импорта сертификатов.

![](pic/03.png)

4\. Выберите файл созданный в предыдущем шаге, с расширением pfx:

![](pic/04.png)


5\. Укажите пароль для файла pfx, если ставили на предыдущем шаге.

![](pic/04-1.png)

6\. Выберите опцию "Автоматически выбрать хранилище на основе типа сертификата". Это позволит сертификату быть использованым для подтверждения сервера и клиента.

![](pic/04-2.png)

7\. После завершения импорта, следует убедиться что сертифкат будет использоваться для аутентификации. 
Следует открыть дополнительные свойства в окне:

![](pic/04-4.png)

В свойствах сертифката должен стоят флаг "Проверка подлинности клиента".

![](pic/04-3.png)

|| Для macOS потребуется установка сертификата в хранилище сертифкатов через Связку ключей. Обзательно в настройках сертификата включить доверие к самому сертификату: ![](pic/04-5.png)

8\. Перейдите по адресу https://netology.local. Убедитесь, что вам предлагают выбрать клиентский сертификат и в строке
подключений нет никаких предупреждений о неправильно установленном сертификате.

![](pic/05.png)

9\. Зайдите в настройки сертификата.

![](pic/06.png)

10\. Сделайте скриншот информации о сертификате.

![](pic/07.png)

11\. Удалите клиентский сертификат — вкладка Your certificates. Перезапустите браузер и убедитесь, что получите ошибку, как на скрине.
