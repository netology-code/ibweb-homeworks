...
## Работа с GitVerse
В этом задании применяется сервис GitVerse — платформа для хостинга Git-репозиториев, контроля версий кода и совместной разработки, являющийся одним из популярных решений для поддержки DevOps и DevSecOps.
...
6. Далее выберите предпочитительный почтовый ящик для получения уведомлений от сервера и имя пользователя.
...
Для добавления учебного проекта в ваш репозиторий небходимо:

1. На верхней панели элементов нажать на **+ Добавить**, **Импорт репозитория**
...
2. Затем выбрать сервис, из которого добавляется репозиторий (github)
3. В поле **URL для импорта** указать адрес:  `https://github.com/netology-code/ib-devsecops-app.git`(при необходимости обновите ссылку)
4. В поле **Название репозитория** задать имя репозитория. Остальные поля опциональны и не требует заполнения.
...
2) Для демонстрации пайплайна можно воспользоваться преимуществами gitverse и использовать облачнный раннер, запускать который не требуется (Подробнее см. [здесь](https://gitverse.ru/docs/actions-conf/cloud-runners-uc/)). В случае использвания облачного раннера допускается использовать сборку произвольного проекта.
